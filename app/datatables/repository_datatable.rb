...
class RepositoryDatatable < AjaxDatatablesRails::Base
  ...
  def get_raw_records
    ...
    if @my_module
      @assigned_rows =
        RepositoryRow
        .joins(:my_modules_repository_rows)
        .where('my_modules_repository_rows.my_module_id': @my_module)
    end
    ...
    if params[:order].present? && params[:order].length == 1
      if sort_column(params[:order].values[0]) == ASSIGNED_SORT_COL
        ...
        if @my_module
          ...
          records.order("repository_cells.id NULLS #{direction}")
        end
        ...
        RepositoryRow.find_by_sql("SELECT repository_rows.*, repository_text_values.data AS value FROM repository_rows
                                  LEFT OUTER JOIN repository_cells ON repository_cells.repository_row_id = repository_rows.id
                                  LEFT OUTER JOIN repository_text_values ON (repository_text_values.id = repository_cells.value_id AND repository_cells.repository_column_id=#{column_id})
